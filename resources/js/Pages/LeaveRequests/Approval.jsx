...
const TYPE_CFG = {
    cuti: { label: 'Cuti', bg: 'bg-purple-50 dark:bg-purple-950/40', text: 'text-purple-600 dark:text-purple-300', border: 'border-purple-100 dark:border-purple-800/50' },
    izin_pribadi: { label: 'Izin Pribadi', bg: 'bg-amber-50 dark:bg-amber-950/40', text: 'text-amber-600 dark:text-amber-300', border: 'border-amber-100 dark:border-amber-800/50' },
    izin_dinas_luar: { label: 'Izin Dinas Luar', bg: 'bg-sky-50 dark:bg-sky-950/40', text: 'text-sky-600 dark:text-sky-300', border: 'border-sky-100 dark:border-sky-800/50' },
    izin_pulang_cepat: { label: 'Izin Pulang Cepat', bg: 'bg-teal-50 dark:bg-teal-950/40', text: 'text-teal-600 dark:text-teal-300', border: 'border-teal-100 dark:border-teal-800/50' },
    sakit: { label: 'Sakit', bg: 'bg-rose-50 dark:bg-rose-950/40', text: 'text-rose-600 dark:text-rose-300', border: 'border-rose-100 dark:border-rose-800/50' },
};
...
export default function Approval({ leaveRequests }) {
    ...
    return (
        <AuthenticatedLayout header={
            <div><div className="flex items-center space-x-2 mb-2"><span className="px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-100 dark:border-indigo-800/50 text-indigo-700 dark:text-indigo-300 text-[10px] font-black uppercase tracking-widest flex items-center shadow-sm"><ClipboardCheck className="w-3 h-3 mr-1.5" /> Persetujuan</span></div>
            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Persetujuan <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-cyan-600 dark:from-indigo-400 dark:to-cyan-400">Cuti & Izin</span></h2></div>
        }>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.3}} className="flex flex-wrap items-center gap-3 mb-6">
                <div className="flex items-center gap-2 px-4 py-2.5 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-sm">
                ...
                <Select value={filterMonth} onValueChange={setFilterMonth}>
                    <SelectTrigger className="w-[150px] rounded-xl border-slate-200 dark:border-slate-800 h-10 font-semibold bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent className="rounded-xl shadow-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                        <SelectItem value="all" className="font-semibold text-slate-900 dark:text-slate-100">Semua Bulan</SelectItem>
                        {MONTHS.map((m,i) => <SelectItem key={i} value={String(i+1)} className="font-semibold text-slate-900 dark:text-slate-100">{m}</SelectItem>)}
                    </SelectContent>
                </Select>
                <Select value={filterYear} onValueChange={setFilterYear}>
                    <SelectTrigger className="w-[120px] rounded-xl border-slate-200 dark:border-slate-800 h-10 font-semibold bg-white/80 dark:bg-slate-900/80 text-slate-900 dark:text-slate-100 text-sm"><SelectValue /></SelectTrigger>
                    <SelectContent className="rounded-xl shadow-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                        <SelectItem value="all" className="font-semibold text-slate-900 dark:text-slate-100">Semua Tahun</SelectItem>
                        {years.map(y => <SelectItem key={y} value={String(y)} className="font-semibold text-slate-900 dark:text-slate-100">{y}</SelectItem>)}
                    </SelectContent>
                </Select>
                {(filterMonth !== String(now.getMonth()+1) || filterYear !== String(now.getFullYear())) && (
                    <button onClick={() => { setFilterMonth(String(now.getMonth()+1)); setFilterYear(String(now.getFullYear())); }} className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 underline underline-offset-2">Reset</button>
                )}
            </motion.div>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {filterBtns.map((fb) => (
                    <button key={fb.key} onClick={()=>setStatusFilter(fb.key)}
                        className={`bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border rounded-2xl p-5 shadow-[0_4px_20px_rgb(0,0,0,0.03)] text-left transition-all hover:-translate-y-0.5 ${statusFilter===fb.key?'border-indigo-300 dark:border-indigo-500/50 ring-2 ring-indigo-100 dark:ring-indigo-950':'border-slate-200/80 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'}`}>
                        <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">{fb.label}</p>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.5,delay:0.1}} className="pb-8">
                <Card className="border border-slate-200/80 dark:border-slate-800 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] bg-white/80 dark:bg-slate-900/90 backdrop-blur-xl overflow-hidden">
                    ...
                    <CardContent className="p-0"><div className="overflow-x-auto"><Table>
                        ...
                        <TableBody><AnimatePresence>
                            ...
                            {filtered.length===0 && <TableRow><TableCell colSpan={7} className="text-center py-16"><div className="flex flex-col items-center text-slate-400 dark:text-slate-500"><CalendarDays className="w-12 h-12 mb-4 text-slate-200 dark:text-slate-700" /><p className="font-bold text-slate-500 dark:text-slate-400">Tidak ada data pengajuan</p></div></TableCell></TableRow>}
                ...
                {totalPages > 1 && (
                    <div className="flex justify-center mt-6">
                        <div className="flex items-center gap-1 bg-white/60 dark:bg-slate-900/60 p-2 rounded-2xl shadow-sm border border-white/50 dark:border-slate-800 backdrop-blur-sm">
                            <button 
